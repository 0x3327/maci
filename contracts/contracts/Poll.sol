...
import {Utilities, IVerifier} from "./utilities/Utility.sol";
    ...
    EmptyBallotRoots
{
    ...
    address immutable verifierContract;
    ...
    string constant ERROR_NOT_PART_OF_STATE_AQ = "PollE07";

    event Join(Message _nullifier, PubKey _newKey);
    ...
    function join(
        Proof memory _proof,
        Message memory _nullifier,
        PubKey memory _newKey
    ) public isWithinVotingDeadline {
        require(
            IVerifier(verifierContract).verifyProof(
                _proof.a,
                _proof.b,
                _proof.c,
                _proof.input
            ),
            ERROR_NOT_PART_OF_STATE_AQ
        );
        require(
            numMessages <= maxValues.maxMessages,
            ERROR_MAX_MESSAGES_REACHED
        ...
        _nullifier.msgType = 3;
        uint256 messageLeaf = hashMessageAndEncPubKey(_nullifier, _newKey);
        extContracts.messageAq.enqueue(messageLeaf);

        emit Join(_nullifier, _newKey);
    }
